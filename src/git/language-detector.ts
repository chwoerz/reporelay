/**
 * Detect the primary programming language(s) of a project directory
 * by checking for well-known manifest/config files.
 *
 * Used at MCP startup to auto-populate the language filter when
 * MCP_LANGUAGES is not explicitly set, so only repos matching
 * the host project's language(s) are served.
 */
